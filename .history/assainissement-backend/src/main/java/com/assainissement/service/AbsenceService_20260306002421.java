...
public class AbsenceService {
    ...
    private static final int MINIMUM_DAYS_IN_ADVANCE = 7;
    ...
    public AbsenceDTO createAbsence(AbsenceDTO absenceDTO) {
        ...
        // Validate: day-off must be requested at least 7 days in advance
        LocalDate today = LocalDate.now();
        long daysUntilStart = ChronoUnit.DAYS.between(today, absenceDTO.getStartDate());
        
        if (daysUntilStart < MINIMUM_DAYS_IN_ADVANCE) {
            throw new RuntimeException(
                    "Les demandes de congé doivent être soumises au moins " + 
                    MINIMUM_DAYS_IN_ADVANCE + " jours à l'avance. " +
                    "Votre demande est pour dans " + daysUntilStart + " jours."
            );
        }
