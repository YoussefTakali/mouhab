...
export class MapComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private readonly missionMarkerMap = new Map<number, L.Marker>();
  private readonly workerMarkerMap = new Map<number, L.Marker>();
  ...
  private initMap(): void {
    ...
    this.ngZone.runOutsideAngular(() => {
      ...
      L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        ...
        keepBuffer: 4              // Keep 4 tiles beyond the viewport for smoother panning
      }).addTo(this.map);
      ...
      // Multi-stage invalidation to ensure tiles load fully:
      // Stage 1: when map fires its 'load' event
      this.map.whenReady(() => {
        this.map.invalidateSize({ animate: false });
      });

      // Stage 2: ResizeObserver for ongoing container changes (panel toggle etc.)
      this.resizeObserver = new ResizeObserver(() => {
        this.map.invalidateSize({ animate: false });
      });
      this.resizeObserver.observe(container);

      // Stage 3: requestAnimationFrame chain — catches cases where the DOM hasn't painted yet
      ...
      // Stage 4: final fallback after 500ms (covers slow CSS transitions / layout shifts)
      setTimeout(() => {
        this.map.invalidateSize({ animate: false });
      }, 500);
  ...
  togglePanel(): void {
    ...
    // Panel transition takes ~300ms; invalidate at key moments
    if (this.map) {
