...
import { Mission, Photo, PhotoType, MISSION_STATUS_LABELS, MISSION_TYPE_LABELS, MISSION_PRIORITY_LABELS } from '../../../core/models/mission.model';
    ...
    <div class="mission-detail-page" *ngIf="mission()">
      ...
      <div class="content-grid">
          ...
          <div class="info-grid">
            ...
            <div class="info-item">
              ...
              <div class="info-content">
                ...
                <span class="info-value">{{ mission()!.assignedToName || mission()!.assignedEmployeeName || 'Non assigné' }}</span>
              </div>
            </div>

            <div class="info-item" *ngIf="mission()!.estimatedDuration">
              <span class="info-icon"><i class="fa-solid fa-stopwatch"></i></span>
              <div class="info-content">
            ...
            <div class="info-item">
              <span class="info-icon"><i class="fa-solid fa-trophy"></i></span>
              <div class="info-content">
        ...
        <div class="info-card checklist-card" *ngIf="mission()!.checklist && mission()!.checklist!.length > 0">
          <h3><i class="fa-solid fa-list-check"></i> Liste de Contrôle</h3>
          <div class="checklist">
        ...
        <div class="info-card photos-card">
          ...
          <div class="photos-section">
            <div class="photos-header">
              ...
              <button class="btn btn-outline btn-sm" *ngIf="canUploadPhotos()" (click)="beforeFileInput.click()">
                <i class="fa-solid fa-cloud-arrow-up"></i> Ajouter photo avant
              </button>
              <input #beforeFileInput type="file" accept="image/*" class="hidden-file-input" (change)="onUploadPhoto($event, 'BEFORE')" />
            </div>
            <div class="photos-grid" *ngIf="beforePhotos().length > 0">
              @for (photo of beforePhotos(); track photo.id) {
                <div class="photo-item" (click)="openPhoto(photo)">
                  <img [src]="getPhotoSrc(photo.id)" [alt]="photo.description || 'Photo avant'" />
          ...
          <div class="photos-section">
            <div class="photos-header">
              ...
              <button class="btn btn-outline btn-sm" *ngIf="canUploadPhotos()" (click)="afterFileInput.click()">
                <i class="fa-solid fa-cloud-arrow-up"></i> Ajouter photo après
              </button>
              <input #afterFileInput type="file" accept="image/*" class="hidden-file-input" (change)="onUploadPhoto($event, 'AFTER')" />
            </div>
            <div class="photos-grid" *ngIf="afterPhotos().length > 0">
              @for (photo of afterPhotos(); track photo.id) {
                <div class="photo-item" (click)="openPhoto(photo)">
                  <img [src]="getPhotoSrc(photo.id)" [alt]="photo.description || 'Photo après'" />
                  <div class="photo-overlay">
                    <span class="photo-time">{{ photo.capturedAt | date:'dd/MM HH:mm' }}</span>
                  </div>
                </div>
        ...
        <div class="info-card timeline-card">
          <h3><i class="fa-solid fa-timeline"></i> Historique</h3>
          <div class="timeline">
            ...
            <div class="timeline-item" *ngIf="mission()!.assignedAt">
              ...
              <div class="timeline-content">
                <span class="timeline-title">Assignée à {{ mission()!.assignedToName || mission()!.assignedEmployeeName }}</span>
      ...
      <div class="action-bar">
        @switch (mission()!.status) {
          ...
          @case ('ASSIGNED') {
            <button class="btn btn-primary" *ngIf="isAssignedToMe() || isSupervisor()" (click)="startMission()">
              <i class="fa-solid fa-play"></i> Démarrer
            </button>
          }
          @case ('IN_PROGRESS') {
            <button class="btn btn-success" *ngIf="isAssignedToMe() || isSupervisor()" (click)="completeMission()">
...
export class MissionDetailComponent implements OnInit {
  ...
  loadPhotos(missionId: number): void {
    this.photoService.getBeforePhotos(missionId).subscribe({
      next: (photos) => {
        this.beforePhotos.set(photos);
        this.prefetchPhotoUrls(photos);
      }
    });
    this.photoService.getAfterPhotos(missionId).subscribe({
      next: (photos) => {
        this.afterPhotos.set(photos);
        this.prefetchPhotoUrls(photos);
      }
    });
  }
  ...
  getPhotoSrc(photoId: number): string {
    return this.photoUrls()[photoId] || this.getPhotoUrl(photoId);
  }

  isAssignedToMe(): boolean {
    const mission = this.mission();
    const employee = this.currentEmployee();
    return (mission?.assignedToId === employee?.id) || (mission?.assignedEmployeeId === employee?.id);
  }
  ...
  startMission(): void {
    this.missionService.updateStatus(this.mission()!.id, 'IN_PROGRESS').subscribe({
      ...
      error: (err) => console.error('Failed to start mission:', err)
    });
  }

  completeMission(): void {
    this.missionService.updateStatus(this.mission()!.id, 'COMPLETED').subscribe({
      next: (mission) => this.mission.set(mission),
      error: (err) => console.error('Failed to complete mission:', err)
    });
  }
  ...
  openPhoto(photo: Photo): void {
    window.open(this.getPhotoSrc(photo.id), '_blank');
  }

  onUploadPhoto(event: Event, type: PhotoType): void {
    ...
    this.photoService.uploadPhoto({
      ...
      next: () => {
        this.loadPhotos(this.mission()!.id);
        input.value = '';
      },
      error: (err) => {
  ...
  prefetchPhotoUrls(photos: Photo[]): void {
    photos.forEach(photo => {
      if (this.photoUrls()[photo.id]) return;
      this.photoService.getPhotoBlob(photo.id).subscribe({
        next: (blob) => {
          const url = URL.createObjectURL(blob);
          this.photoUrls.set({ ...this.photoUrls(), [photo.id]: url });
        },
