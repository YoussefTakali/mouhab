...
@Component({
  ...
  template: `
    <div class="employee-detail-page" *ngIf="employee()">
      ...
      <div class="content-grid">
        ...
        <div class="info-card">
          ...
          <div class="info-list">
            ...
            <div class="info-item" *ngIf="employee()?.phone">
              <span class="info-icon"><i class="fa-solid fa-mobile-screen"></i></span>
              <div>
        ...
        <div class="info-card stats-card">
          ...
          <div class="stats-grid">
            <div class="stat-box">
              ...
              <span class="stat-label"><i class="fa-solid fa-trophy"></i> Points</span>
            </div>
            <div class="stat-box">
              <span class="stat-value">{{ employee()?.completedMissions || 0 }}</span>
              <span class="stat-label"><i class="fa-solid fa-circle-check"></i> Missions</span>
            </div>
            <div class="stat-box">
              <span class="stat-value">#{{ employee()?.leaderboardRank || '-' }}</span>
              <span class="stat-label"><i class="fa-solid fa-ranking-star"></i> Rang</span>
            </div>
            <div class="stat-box">
              <span class="stat-value">{{ employee()?.avgRating?.toFixed(1) || '-' }}</span>
              <span class="stat-label"><i class="fa-solid fa-star"></i> Note</span>
            </div>
          </div>
        </div>
        ...
        <div class="info-card missions-card">
          <h2><i class="fa-solid fa-bullseye"></i> Missions Récentes</h2>
    ...
    <div class="error-state" *ngIf="!isLoading() && !employee()">
      <span class="error-icon"><i class="fa-solid fa-face-frown"></i></span>
      ...
      <div class="modal-overlay" (click)="closeEditModal()">
        <div class="modal-content" (click)="$event.stopPropagation()">
          <div class="modal-header">
            <h2><i class="fa-solid fa-user-pen"></i> Modifier le profil</h2>
          ...
          <form (ngSubmit)="saveProfile()">
              ...
              <div class="skills-picker">
                @for (skill of availableSkills; track skill) {
                  <label class="skill-checkbox">
                    <input type="checkbox" [checked]="editForm.skills?.includes(skill)" 
            ...
            <div class="form-group full-width">
              ...
              <div class="certifications-input">
                <input type="text" [(ngModel)]="newCertification" name="newCertification" 
                       placeholder="Ajouter une certification..." 
                       (keydown.enter)="addCertification(); $event.preventDefault()" />
                <button type="button" class="btn btn-sm btn-primary" (click)="addCertification()">
                  <i class="fa-solid fa-plus"></i>
                </button>
              </div>
              <div class="certification-tags">
                @for (cert of editForm.certifications || []; track cert) {
                  <span class="cert-tag">
                    {{ cert }}
                    <button type="button" (click)="removeCertification(cert)">
                      <i class="fa-solid fa-xmark"></i>
                    </button>
                  </span>
                }
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-outline" (click)="closeEditModal()">Annuler</button>
  ...
  styles: [`
    ...
    .status-available, .status-active, .status-online { 
    ...
    .status-absent, .status-unavailable, .status-offline { 
...
export class EmployeeDetailComponent implements OnInit {
  ...
  getStatusLabel(): string {
    ...
    const isOnline = emp.user?.online ?? false;
    
    if (!isOnline) {
      return 'Hors ligne';
    }
    
    // If online, check if has specific status
    if (emp.status) {
      ...
      return labels[emp.status] || emp.status;
    }
  ...
  toggleSkill(skill: string): void {
    ...
    this.editForm.skills = [...skills];
  }

  addCertification(): void {
    if (!this.newCertification.trim()) return;
    
    const certs = this.editForm.certifications || [];
    if (!certs.includes(this.newCertification.trim())) {
      certs.push(this.newCertification.trim());
      this.editForm.certifications = [...certs];
    }
    this.newCertification = '';
  }
  ...
  removeCertification(cert: string): void {
    const certs = this.editForm.certifications || [];
    const index = certs.indexOf(cert);
    if (index > -1) {
  ...
  saveProfile(): void {
    ...
    const updateData: Partial<Employee> = {
      ...
      contractType: this.editForm.contractType as any,
      user: {
        id: emp.user?.id || 0,
        ...
        active: emp.user?.active ?? true,
        online: emp.user?.online ?? false,
        fullName: `${this.editForm.firstName} ${this.editForm.lastName}`
      }
    };

    // Determine which endpoint to use
    const isOwnProfile = emp.user?.id === this.authService.currentUser()?.id;
    
    const request$ = isOwnProfile && !this.canManageEmployees()
      ? this.employeeService.updateMyProfile(updateData)
      : this.employeeService.updateEmployee(emp.id, updateData);
