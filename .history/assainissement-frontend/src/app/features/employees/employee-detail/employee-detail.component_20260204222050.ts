...
export class EmployeeDetailComponent implements OnInit {
  ...
  loadRecentMissions(employeeId: number): void {
    this.missionService.getMissions({ employeeId, limit: 5 }).subscribe({
      next: (missions) => this.recentMissions.set(missions),
  ...
  getAvatarColor(): string {
    ...
    const index = (emp.firstName.charCodeAt(0) + emp.lastName.charCodeAt(0)) % colors.length;
    return colors[index];
  }
