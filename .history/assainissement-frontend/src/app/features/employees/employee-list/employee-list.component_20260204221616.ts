...
@Component({
  ...
  styles: [`
    ...
    .status-indicator.status-absent { background: var(--warning-color); }
...
export class EmployeeListComponent implements OnInit {
  ...
  getAvatarColor(employee: Employee): string {
    ...
    const index = (employee.firstName.charCodeAt(0) + employee.lastName.charCodeAt(0)) % colors.length;
    return colors[index];
  }

  getEmployeeStatus(employee: Employee): string {
    // This would normally check actual status from backend
    return 'available';
  }
