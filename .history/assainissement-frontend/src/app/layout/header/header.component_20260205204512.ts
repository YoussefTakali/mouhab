...
@Component({
  ...
  styles: [`
    .navbar {
      ...
      right: 0;
      height: 64px;
      background: linear-gradient(135deg, #0d47a1 0%, #1e88e5 50%, #1565c0 100%);
      display: flex;
      align-items: center;
      padding: 0 1rem;
      z-index: 1001;
...
export class HeaderComponent implements OnInit, OnDestroy {
  ...
  loadNotifications(): void {
    ...
    if (employee) {
      // Workers/Supervisors: get missions assigned to them that need attention
      this.missionService.getMissionsByEmployee(employee.id).subscribe({
        next: (missions) => {
          // Show assigned, in-progress, and awaiting approval missions
          this.notifications.set(
            missions.filter(m =>
              ['ASSIGNED', 'IN_PROGRESS', 'AWAITING_APPROVAL', 'PENDING'].includes(m.status)
            ).slice(0, 10)
          );
        },
      ...
      // Supervisors/Admins: show all pending/awaiting missions
      this.missionService.getAllMissions().subscribe({
        next: (missions) => {
          this.notifications.set(
            missions.filter(m =>
              ['PENDING', 'AWAITING_APPROVAL'].includes(m.status)
            ).slice(0, 10)
          );
        },
        error: () => this.notifications.set([])
