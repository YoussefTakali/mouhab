...
public class DataInitializer implements CommandLineRunner {
    ...
    private void initializeAccounts() {
        ...
        private void ensureEmployeeProfiles() {
                log.info("Ensuring employee profiles for technicians...");
                userRepository.findByRole(UserRole.WORKER).forEach(this::createEmployeeIfMissing);
                userRepository.findByRole(UserRole.SUPERVISOR).forEach(this::createEmployeeIfMissing);
        }

        private void createEmployeeIfMissing(User user) {
                ...
                Employee employee = Employee.builder()
                                .user(user)
                                .employeeCode("EMP" + String.format("%05d", user.getId()))
                                .totalPoints(0)
                                .monthlyPoints(0)
